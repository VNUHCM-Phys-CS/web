...
class Helper
{
    ...
    public function getVideoLink($content){
        $paternlink = '/<source.*?src="(.*?\.(?:mp4|mkv|webm))"/i';
        preg_match($paternlink, $content, $data);
        return isset($images[1]) ? $data[1] : "";
    }
    public function getAudioLink($content){
        $paternlink = '/(http|https):\/\/.*\/(.*)\.(mp3|mp4)/';
        preg_match($paternlink, $content, $jsData);
        return isset($jsData[0]) ? $jsData[0] : "" ;
    }
    public function getYoutubeId($content){
        $paternlink = '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/\s]{11})%i';
        preg_match($paternlink, $content, $data);
        return isset($data[1]) ? $data[1] : "";
    }
    public function getVideo($content){
        $data = ['type' => false,'link' => '','classPop' => ''];
        if($ytId = self::getYoutubeId($content)){
            $data['classPop'] = 'popup-youtubeT';
            $data['type'] = 'youtube';
            $data['link'] = "https://www.youtube.com/watch?v={$ytId}";
        }elseif($video = self::getVideoLink($content)) {
            $data['classPop'] = 'mfp-video';
            $data['type'] = 'video';
            $data['link'] = $video;
        }
        return $data;
    }
    public function getDocsLink($content){
        $paternlink = '/<source.*?src="(.*?\.(?:pdf|docx|doc|xlsx|xls))"/i';
        preg_match($paternlink, $content, $data);
        return isset($data[1]) ? $data[1] : "";
    }
}
